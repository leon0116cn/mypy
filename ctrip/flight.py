...
def get_post_data(dcity='SHA', acity='SIA', days=0, flight_way='Oneway', class_type='ALL', **kwargs):
    airport_params = []
    qdate = (datetime.now() + timedelta(days=days)).strftime('%Y-%m-%d')
    airport_params.append(dict(dcity=dcity, acity=acity, date=qdate))
    post_data = dict(flightWay=flight_way, classType=class_type, searchIndex=1, airportParams=airport_params,
                     token='536bc24712ea27bb5109f78178533ba7')
    if kwargs and 'airline' in kwargs:
        post_data['airline'] = kwargs['airline']
    if kwargs and 'hasChild' in kwargs:
        post_data['hasChild'] = kwargs['hasChild']
    if kwargs and 'hasBaby' in kwargs:
        post_data['hasBaby'] = kwargs['hasBaby']
    return post_data
...
def show_request_detail(response):
    print('Request url is : {0}'.format(response.url))
    print('Request body is {0}:'.format(response.request.body))
...
def parser_route_list(route_list, route_data):
    for route in route_list:
        if route.get('routeType') == 'Flight':
            ...
            route_data.append((airline_name, flight_number, lowest_price, departure_date, departure_airport_name,
